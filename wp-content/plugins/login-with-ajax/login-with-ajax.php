...
class LoginWithAjax {
	...
	public static function login(){
		...
		if( !empty($_REQUEST['log']) && !empty($_REQUEST['pwd']) && trim($_REQUEST['log']) != '' && trim($_REQUEST['pwd'] != '') ){
			...
			if ( strtolower(get_class($loginResult)) == 'wp_user' ) {
				...
				$return['result'] = true;
			/*	$return['message'] = __("Успешно!",'login-with-ajax'); */
			...
			$return['result'] = false;
			$return['error'] = __('Введите электропочту и пароль', 'login-with-ajax');
		}
		$return['action'] = 'login';
	...
	public static function register(){
	    ...
	    if( get_option('users_can_register') ){
			...
			$subscriber = $_REQUEST['user_email'];

			if ( !is_wp_error($errors) ) {
				//Success
				$return['result'] = true;
				$return['message'] = __('<h2>Благодарим за регистрацию <span class="glyphicon glyphicon-ok" aria-hidden="true" style="color:red;"></h2><br> Проверьте Вашу почту, перейдите по ссылке и начните обучение.','login-with-ajax');
				...
				curl_setopt($ch, CURLOPT_USERPWD, 'mchbimvc:86456d3f5960f35bd68cb395631de965-us14');
	    	...
	    	$return['result'] = false;
			$return['error'] = __('Неудачная регистрация. Попробуйте ещё.','login-with-ajax');
	    }
		return $return;
	}
