...
function wp_new_user_notification_original($user_id, $plaintext_pass = '', $include_fields = false, $activation_data = false, $welcome_email = '') {
	...
	$message_headers = "From:<{$admin_email}>\n" . "Content-Type: text/html; charset=\"" . get_option('blog_charset') . "\"\n";
	...
	wp_mail($user_email, sprintf(__('Данные регистрации'), $blogname), $welcome_email, $message_headers);
}
...
function cimy_signup_user_notification($user, $user_email, $key, $meta = '') {
	...
	// Send email with activation link. get_site_option( 'blogname' ) == '' ? 'WordPress' : esc_html( get_site_option( 'blogname' ) )
	$admin_email = get_site_option( 'admin_email' );
	if ( $admin_email == '' )
		$admin_email = 'support@' . $_SERVER['SERVER_NAME'];
	$from_name = 'Vysotskiy Consulting - BIM.VC';
	$message_headers = "From:<{$admin_email}>\n" . "Content-Type: text/html; charset=\"" . get_option('blog_charset') . "\"\n";
	$message = sprintf( apply_filters( 'wpmu_signup_user_notification_email', __( "Привет, вы заполнили анкету в Базе Знаний Vysotskiy Consulting<br><br><h2><a href=\"%s\">Войдите, используя электропочту и пароль</a></h2><br><br>Успехов,<br>Команда Vysotskiy Consulting<br> <a href=\"http://bim.vc\">BIM.VC</a>", $cimy_uef_domain) ), site_url( "auth.php?cimy_key=$key&action=cimy_uef_redirect&redirect_to=/base/video/?modal=login" ), $key );
	// TODO: Don't hard code activation link.
	$subject = sprintf(
		apply_filters('wpmu_signup_user_notification_subject',
			__('Регистрация в Базе Знаний', $cimy_uef_domain),
	...
	if (isset($_GET["cimy_key"])) {
		...
		if ( is_wp_error($result) ) {
			if ( 'already_active' == $result->get_error_code()) {
				...
				$message.= sprintf( __('Your site at <a href="%1$s">%2$s</a> is active. You may now log in to your site using your chosen username of &#8220;%3$s&#8221;.  Please check your email inbox at %4$s for your password and login instructions.  If you do not receive an email, please check your junk or spam folder.  If you still do not receive an email within an hour, you can <a href="%5$s">reset your password</a></p>.', $cimy_uef_domain), 'http://' . $signup->domain, $signup->domain, $signup->user_login, $signup->user_email, network_site_url( 'auth.php?action=lostpassword' ) );
			} else {
...
function cimy_check_user_on_signups($errors, $user_name, $user_email) {
	...
	if ( $signup != null ) {
		...
		if (($diff > 172800) || ($signup->active))
			...
			$errors->add('user_name', __('Имя электронной почты занято - попробуйте другю', $cimy_uef_domain));
