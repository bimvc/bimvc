		...
		foreach ($fields as $thisField) {
			...
			if ((!$rules['can_be_empty']) || (!empty($value))) {
				...
				if ((!$rules['can_be_empty']) && (in_array($type, $rule_canbeempty)) && (empty($value))) {
					...
					if ($empty_error)
						$errors->add($unique_id, 'Заполните все поля анкеты '.__('', $cimy_uef_domain));
				}
	...
	while ($i <= 2) {
		...
		foreach ($fields as $thisField) {
			...
			switch($type) {
				...
				case "password":
				case "text":
					$obj_label = ''.cimy_uef_sanitize_content($label).'';
					$obj_class = ' class="form-control '.$input_class.$obj_class.'"';
...
?>
		<form name="registerform" id="registerform" action="<?php echo site_url('/login/index.php?action=register', 'login_post') ?>" method="post">
<?php
