...
function wpa_front_end_login(){
	...
	if( $account && !is_wp_error($sent_link) ){
		echo '<p class="wpa-box wpa-success">'. apply_filters('wpa_success_link_msg', __('Ссылка для входа отправлена на Вашу электропочту', 'passwordless') ) .'</p>';
	} elseif ( is_user_logged_in() ) {
		$current_user = wp_get_current_user();
		echo '<p class="wpa-box wpa-alert">'.apply_filters('wpa_success_login_msg', sprintf(__( 'Вы уже вошли', 'profilebuilder' ), '<a href="'.$authorPostsUrl = get_author_posts_url( $current_user->ID ).'" title="'.$current_user->display_name.'">'.$current_user->display_name.'</a>', '<a href="'.wp_logout_url( $redirectTo = wpa_curpageurl() ).'" title="'.__( 'Log out of this account', 'passwordless' ).'">'. __( 'Log out', 'passwordless').' &raquo;</a>' ) ) . '</p><!-- .alert-->';
	} else {
		if ( is_wp_error($sent_link) ){
			echo '<p class="wpa-box wpa-error">' . apply_filters( 'wpa_error', $sent_link->get_error_message() ) . '</p>';
		}
		if( $error_token ) {
			echo '<p class="wpa-box wpa-error">' . apply_filters( 'wpa_invalid_token_error', __('Ваша ссылка устарела. Восстановите пароль ещё раз.', 'passwordless') ) . '</p>';
		}
		?>
		
		<form name="wpaloginform" id="open" action="" method="post" class="fclose" >
			<input type="text" name="user_email_username" id="user_email_username" class="form-control" placeholder="Введите электропочту" >
			По этому адресу будет отправлено письмо со ссылкой для входа в Базу Знаний
			<br><br>
			<input type="submit" name="wpa-submit" id="wpa-submit" class="btn btn-success btn-lg" value="Получить доступ">
			<input id="nonce" name="login-with-ajax" value="124f1fe55b" type="hidden">
...
function wpa_send_link( $email_account = false, $nonce = false ){
	...
	if (is_wp_error($valid_email)){
		...
		$subject = apply_filters('wpa_email_subject', __('Вход в Базу Знаний BIM.VC'));
		$message = apply_filters('wpa_email_message', __('Привет, Вы забыли пароль для доступа в Базу Знаний<br><br><h2><a href="'.$unique_url.'">Для входа, перейдите по ссылке</a></h2><br><br>Успехов,<br>Команда Vysotskiy Consulting<br> <a href="http://bim.vc" target="_blank">BIM.VC</a>'), $unique_url);	
